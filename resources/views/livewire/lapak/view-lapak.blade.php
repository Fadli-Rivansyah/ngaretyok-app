<div class="max-w-3xl flex flex-col mx-auto sm:px-6 lg:px-8 space-y-4">
    <h1 class="text-5xl capitalize font-extrabold break-words tracking-tight text-gray-900 dark:text-white">{{$lapak->nama_lapak}}</h1>
    <div class="flex items-center">
        <div class="flex items-center gap-2">
            ...
            <span class="bg-indigo-100 text-indigo-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-indigo-900 dark:text-indigo-300">{{date('d F Y', strtotime($lapak->created_at))}}</span>
        </div>
        <hr class="h-full w-1 mx-4 bg-red-300 border-0 rounded"/>
        <div class="flex items-center gap-2">
            @if(date('Y-m-d') >= date('Y-m-d', strtotime($lapak->tanggal_lapak)))
            ...
            @else
                <span class="bg-red-100 text-red-800  text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-indigo-900 dark:text-indigo-300">{{date('d F Y', strtotime($lapak->tanggal_lapak))}}</span>
            @endif
        </div>
    </div>
    <img class="object-cover w-full rounded-lg" src="{{ Storage::url($hiro) }}" alt="belum-tampil">
    <p class="text-lg font-normal text-gray-500 lg:text-xl dark:text-gray-400">{{$lapak->deskripsi_lapak}}</p>
    <div class="flex flex-col mt-[5em] ">
        ...
        <div class="grid lg:grid-cols-2 gap-4 my-5">
            @foreach ($images as $item)
                <img class="h-auto w-full rounded-lg" src="{{Storage::url($item->path)}}"  alt="gambar">
            @endforeach
            ...
            {{-- modal --}}
            <div @if(!$isOpen) inert @endif class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            @if ($isOpen)
                                        ...
                                        @elseif($images)
                                            <div class="flex flex-wrap gap-2 ">
                                                ...
                                                <div  class="flex flex-row items-center w-max max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800">
                                                    <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:bg-blue-800 dark:text-blue-200">
                                                        <img src="{{ Storage::url($item->path) }}" class="max-w-11" alt="preview-img">
                                                    </div>
                                                    <button wire:click="deleteImage({{$index}})" type="button" id="delete-lapak" class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" >
                                                        <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                                        </svg>
                                                    </button>
