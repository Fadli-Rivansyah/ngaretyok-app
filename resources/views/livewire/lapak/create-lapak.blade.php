<div class="max-w-7xl  mx-auto md:w-2/3 lg:w-1/2 sm:px-6 lg:px-8 space-y-4">
    <form wire:submit.prevent="save" class="w-full bg-white mx-auto shadow-md rounded-lg px-8 pt-6 pb-8 mb-4">
        <h1 class="mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-2xl lg:text-3xl dark:text-white">{{$header}}</h1>
        <div class="mb-4">
            ...
            @error('nama_lapak') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            ...
            <div class="relative mb-4">
                ...
                @error('tanggal_lapak') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
        </div>
        <div class="mb-4">
            ...
            @error('foto_lapak') <span class="text-red-500 text-sm"></span> @enderror
            @if($foto_lapak)
                <div class="flex flex-wrap gap-2 ">
                @foreach ($foto_lapak as $index => $photo)
                    @if ($photo->temporaryUrl()) <!-- Periksa jika URL sementara ada -->
                        <div id="toast-default" class="flex items-center w-max max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
                            <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:bg-blue-800 dark:text-blue-200">
                                <img src="{{ $photo->temporaryUrl() }}" class="max-w-11" alt="preview-img">
                            </div>
                            <button wire:click="deletePreview({{$index}})" type="button" class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-default" aria-label="Close">
            ...
            @elseif($previewUpdate)
                <div class="flex flex-wrap gap-2 ">
                    @foreach ($previewUpdate as $index => $item)
                        <div id="toast-default" class="flex items-center w-max max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
                            <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-blue-500 bg-blue-100 rounded-lg dark:bg-blue-800 dark:text-blue-200">
                                <img src="{{ Storage::url($item->path) }}" alt="gambar" class="max-w-11" alt="preview-img">
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="mb-4">
            ...
            @error('deskripsi_lapak') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        {{-- indicator --}}
        <div class="my-2 mb-4">
            ...
            <ul class="w-full grid lg:grid-cols-2 space-y-2 text-gray-500 list-inside dark:text-gray-400">
                <li class="flex items-center">
                    <svg  class="w-4 h-4 me-2 {{ strlen($nama_lapak) > 0 ? 'text-green-500' : 'text-slate-500' }} flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
                    </svg>
                    Nama lapak harus mudah diingat
                </li>
                <li class="flex items-center">
                    <svg  class="w-4 h-4 me-2 {{ strlen($tanggal_lapak) > 0 ? 'text-green-500' : 'text-slate-500' }} flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
                    </svg>
                    Tanggal sesuai dengan keadaan waktu
                </li>
                <li class="flex items-center">
                    <svg class="w-4 h-4 me-2 {{ $foto_lapak || $previewUpdate ? 'text-green-500' : 'text-slate-500' }} flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
                    </svg>
                    Upload gambar lapak 
                </li>
                <li class="flex items-center">
                    <svg  class="w-4 h-4 me-2 {{ strlen($deskripsi_lapak) > 0 ? 'text-green-500' : 'text-slate-500' }} flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
                    </svg>
